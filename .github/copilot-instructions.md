...
This repository is a teaching demo for a simple live model-backed multi-agent solution in a C# 10 console application. The code should remain small, readable, and easy to explain in a classroom, workshop, or mentoring session.
...
- Keep the live model-backed agent behavior easy to follow: each agent should have one clear instruction block and one clear responsibility.
...
- Do not remove the live Foundry-backed agent calls unless the user asks to return to a deterministic version.